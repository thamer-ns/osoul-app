    ...
    try:
        ...
        for date, row in df.iterrows():
            ...
            execute_query("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (symbol, 'Annual', d_str, row['revenue'], row['net_income'], row['gross_profit'], row.get('operating_income', 0), row.get('total_assets',0), row.get('total_liabilities',0), row.get('total_equity',0), row.get('operating_cash_flow',0), row.get('free_cash_flow',0), row.get('eps', 0), 'Yahoo'))
        return True
    except Exception as e:
...
def save_thesis(symbol, text, target, rec):
    ...
    execute_query("INSERT OR REPLACE INTO InvestmentThesis (symbol, thesis_text, target_price, recommendation, last_updated) VALUES (?, ?, ?, ?, ?)", (symbol, text, target, rec, now))
