...
import io
import pdfplumber  # مكتبة التعامل مع PDF
...
def parse_pasted_text(raw_text):
    try:
        ...
        # قلب الجدول (Transpose) لأن أرقام تضع السنوات في الأعمدة
    ...
    try:
        with pdfplumber.open(uploaded_file) as pdf:
            for page in pdf.pages:
                ...
                # هنا يمكن إضافة منطق معالجة الجداول المستخرجة
                # حالياً نعيد قائمة فارغة لأن الهيكل يختلف بين الشركات
                pass
        return [] 
...
def render_financial_dashboard_ui(symbol):
    ...
    with tabs[1]:
        ...
        if pdf_file:
            ...
            st.warning("الاستخراج الآلي من PDF يتطلب مكتبات إضافية، يفضل استخدام النسخ واللصق حالياً للدقة.")
    ...
    if not df.empty:
        ...
        with c_chart:
            ...
            if 'revenue' in df.columns and 'net_income' in df.columns:
                ...
                fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font={'family': "Cairo"}, height=350)
