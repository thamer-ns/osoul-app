...
# ==========================================
# 1. دوال مساعدة (الفرز البسيط القديم)
# ==========================================
def apply_sorting(df, cols_definition, key_suffix):
    """واجهة فرز بسيطة وفعالة"""
    if df.empty: return df
    
    # استخدام قاموس للترجمة العكسية
    label_map = {label: col for col, label in cols_definition}
    
    c1, c2 = st.columns([2, 1])
    with c1:
        # قائمة منسدلة عادية بعنوان عربي
        sort_col = st.selectbox("فرز حسب:", list(label_map.keys()), key=f"sc_{key_suffix}")
    with c2:
        sort_ord = st.selectbox("الترتيب:", ["تنازلي (الأحدث/الأعلى)", "تصاعدي (الأقدم/الأقل)"], key=f"so_{key_suffix}")
        
    target = label_map[sort_col]
    asc = True if "تصاعدي" in sort_ord else False
        ...
        if not open_df.empty:
            ...
            with st.expander("🔻 بيع سهم"):
                with st.form("sell"):
                    ...
                    p = c2.number_input("سعر البيع:", min_value=0.0)
    ...
    with t3:
        with st.expander("💰 توزيع"):
            with st.form("ret"):
                ...
                a = c2.number_input("المبلغ:", 0.0)
                d = st.date_input("التاريخ:", date.today())
                n = st.text_input("النوع:")
                if st.form_submit_button("حفظ"):
...
def view_add_trade():
    ...
    with st.form("add_trade"):
        ...
        prc = c4.number_input("السعر:", 0.0, step=0.01)
...
def view_analysis(fin):
    # استخدام دالة التحليل الفني مع مفاتيح فريدة للحقول
...
def view_backtester_ui(fin):
    ...
    c1, c2, c3 = st.columns(3)
    with c1: 
        sym = st.selectbox("السهم:", list(set(fin['all_trades']['symbol'].unique().tolist()+["1120"])))
    with c2: 
        strt = st.selectbox("استراتيجية:", ["Trend Follower", "Sniper"])
    with c3: 
        cap = st.number_input("رأس المال:", 100000)
    if st.button("🚀 تشغيل"):
...
def view_settings():
    ...
    with st.expander("📥 استيراد بيانات (Excel/CSV)"):
        ...
        if f and st.button("بدء الاستيراد"):
            st.info("الاستيراد جاهز للعمل")
    
    # تم إخفاء خيارات الحذف كما طلبت
