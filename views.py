...
# استيراد الرسوم البيانية
from charts import view_advanced_chart 

# محاولة استيراد التحليل المالي (مع معالجة الخطأ إذا لم يكن الملف موجوداً)
try: 
...
def view_portfolio(fin, page_key):
    ...
    with t1:
        if not open_df.empty:
            # ترتيب تلقائي حسب التاريخ الأحدث فقط (تم حذف الفرز والبيع)
...
def view_add_operations():
    ...
    with tab1:
        with st.form("stock_op"):
            ...
            open_symbols = []
            if not trades.empty:
                # تصفية الأسهم المفتوحة في المحفظة المحددة لجلبها في القائمة
            ...
            if st.form_submit_button("تنفيذ العملية"):
                if not selected_sym or qty <= 0 or price <= 0:
                    ...
                    # منطق البيع (تحديث الصفقة المفتوحة)
                        ...
                        # إغلاق الصفقة بناءً على الرمز والمحفظة
                        execute_query("""
                            UPDATE Trades 
