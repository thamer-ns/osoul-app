...
# ==========================================
# 1. دوال مساعدة للاستيراد (أعدتها لك)
# ==========================================
...
def view_settings():
    ...
    with st.expander("📥 استيراد بيانات (Excel/CSV)"):
        ...
        if f and st.button("بدء الاستيراد"): 
            ...
            # منطق الاستيراد المبسط للتعامل مع أنواع الملفات
            try:
                if f.name.endswith('xlsx'): df = pd.read_excel(f)
                else: df = pd.read_csv(f)
                # تخمين الجدول بناءً على الأعمدة
                tbl = "Trades"
                if 'amount' in df.columns and 'note' in df.columns: tbl = "Deposits"
