...
def apply_sorting(df, cols_definition, key_suffix):
    ...
    with st.expander("🔍 خيارات الفرز", expanded=False):
        ...
        with c1: 
            st.markdown("**فرز حسب:**")
            selected = st.selectbox("s", list(label_to_col.keys()), key=f"sc_{key_suffix}", label_visibility="collapsed")
        with c2: 
            st.markdown("**الترتيب:**")
            order = st.radio("o", ["تنازلي", "تصاعدي"], horizontal=True, key=f"so_{key_suffix}", label_visibility="collapsed")
        ...
        if not open_df.empty:
            ...
            with st.expander("🔴 تسجيل بيع"):
                with st.form(f"sell_{page_key}"):
                    ...
                    st.markdown("**السهم:**"); sel = c1.selectbox("s", open_df['symbol'].unique(), label_visibility="collapsed")
                    st.markdown("**سعر البيع:**"); ep = c2.number_input("p", min_value=0.01, label_visibility="collapsed")
                    st.markdown("**التاريخ:**"); ed = c3.date_input("d", date.today(), label_visibility="collapsed")
...
def view_cash_log():
    ...
    with t1:
        ...
        with st.expander("➕ إيداع جديد"):
             with st.form("dep"):
                 st.markdown("**المبلغ:**"); amt = st.number_input("a", label_visibility="collapsed")
                 st.markdown("**التاريخ:**"); dt = st.date_input("d", label_visibility="collapsed")
                 st.markdown("**ملاحظة:**"); nt = st.text_input("n", label_visibility="collapsed")
    ...
    with t2:
        ...
        with st.expander("➖ سحب جديد"):
             with st.form("wit"):
                 st.markdown("**المبلغ:**"); amt = st.number_input("wa", label_visibility="collapsed")
                 st.markdown("**التاريخ:**"); dt = st.date_input("wd", label_visibility="collapsed")
                 st.markdown("**ملاحظة:**"); nt = st.text_input("wn", label_visibility="collapsed")
...
def view_analysis(fin):
    ...
    with c_search: 
        st.markdown("**بحث جديد:**")
        new_search = st.text_input("s", label_visibility="collapsed")
    if new_search and new_search not in symbols: symbols.insert(0, new_search)
    with c_sel: 
        st.markdown("**اختر الشركة:**")
        symbol = st.selectbox("sl", symbols, label_visibility="collapsed") if symbols else None
...
def view_backtester_ui(fin):
    ...
    with c1: 
        ...
        st.markdown("**السهم:**"); symbol = st.selectbox("bs", syms, label_visibility="collapsed")
    with c2: 
        st.markdown("**الاستراتيجية:**"); strat = st.selectbox("bst", ["Trend Follower", "Sniper"], label_visibility="collapsed")
    with c3: 
...
def view_add_trade():
    ...
    with st.form("add"):
        c1, c2 = st.columns(2)
        st.markdown("**الرمز:**"); sym = c1.text_input("as", label_visibility="collapsed")
        st.markdown("**المحفظة:**"); strat = c2.selectbox("ast", ["استثمار", "مضاربة", "صكوك"], label_visibility="collapsed")
        c3, c4, c5 = st.columns(3)
        st.markdown("**الكمية:**"); qty = c3.number_input("aq", min_value=1.0, label_visibility="collapsed")
        st.markdown("**السعر:**"); price = c4.number_input("ap", min_value=0.0, label_visibility="collapsed")
        st.markdown("**التاريخ:**"); date_ex = c5.date_input("ad", date.today(), label_visibility="collapsed")
...
def view_settings():
    ...
    if uploaded_files and st.button("🚀 بدء الاستيراد"):
        # (منطق الاستيراد المختصر لتوفير المساحة - نفس الموجود سابقاً)
