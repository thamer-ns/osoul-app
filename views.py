...
def view_portfolio(fin, page_key):
    ...
    with t1:
        if not open_df.empty:
            ...
            cols_sec = [('sector', 'القطاع'), ('symbol', 'عدد'), ('total_cost', 'التكلفة'), ('current_weight', 'الوزن الحالي'), ('target_percentage', 'الهدف'), ('remaining', 'المتبقي')]
        ...
        if st.form_submit_button("حفظ العملية"):
            if sym and qty:
                ...
                execute_query(
                    ...
                    (sym, n, s, asset, str(d), qty, price, strat, 'Open', price)
                )
...
def view_analysis(fin):
    ...
    if not symbols: 
        st.info("الرجاء إضافة أسهم للمحفظة أو قائمة المراقبة أولاً للتحليل."); return
    ...
    if symbol:
        ...
        if ratios:
            ...
            k1.metric("مكرر الربحية (P/E)", f"{ratios['P/E']:.2f}" if ratios['P/E'] else "-")
            k2.metric("القيمة الدفترية (P/B)", f"{ratios['P/B']:.2f}" if ratios['P/B'] else "-")
            k3.metric("العائد ع الملكية (ROE)", f"{ratios['ROE']:.1f}%" if ratios['ROE'] else "-")
