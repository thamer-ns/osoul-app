...
# استيراد الوحدات الاختيارية (مع حماية من الأخطاء)
...
except ImportError: view_advanced_chart = lambda s: st.info("الشارت غير متاح")
try: from classical_analysis import render_classical_analysis
except ImportError: render_classical_analysis = lambda s: st.info("غير متاح")
...
def clean_and_fix_columns(df, table_name):
    """دالة الإصلاح السحرية للاستيراد"""
    if df is None: return None
    df.columns = df.columns.str.strip().str.lower()
    
    # خريطة توحيد الأسماء (حل مشكلة source/reason/notes)
    rename_map = {
...
def view_portfolio(fin, page_key):
    ...
    df = all_data[all_data['strategy'].astype(str).str.strip() == target_strat].copy() if not all_data.empty else pd.DataFrame()
    ...
    if not open_df.empty:
        ...
        open_df['gain_pct'] = (open_df['gain'] / (open_df['quantity'] * open_df['entry_price'])) * 100
...
def view_settings():
    ...
    if uploaded_files and st.button("🚀 بدء الاستيراد"):
        success = 0
        status = st.empty()
        table_map = {'trades': 'Trades', 'deposits': 'Deposits', 'withdrawals': 'Withdrawals', 'returns': 'ReturnsGrants', 'watchlist': 'Watchlist'}
