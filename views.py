    ...
    with st.form("add"):
        ...
        if st.form_submit_button("حفظ العملية"):
            if sym and qty:
                ...
                if strat == "صكوك" and asset_val != "Sukuk":
                    asset_val = "Sukuk"
...
def view_settings():
    ...
    # 2. إنشاء جدول واحد فقط (للعرض والتعديل معاً)
    # نستخدم column_config لجعل عمود القطاع للقراءة فقط، وعمود النسبة للتعديل
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            "sector": st.column_config.TextColumn(
                "القطاع",
                disabled=True, # منع تعديل اسم القطاع للحفاظ على الترابط
                width="medium"
            ),
        ...
        num_rows="fixed" # منع إضافة أو حذف صفوف
    )
    ...
    if st.button("حفظ التغييرات"):
        ...
        if total_pct > 100:
            st.warning(f"⚠️ مجموع النسب {total_pct}% أكبر من 100%!")
    ...
    if symbol:
        ...
        with tab_fund:
            ...
            if ratios and ratios.get('Current_Price', 0) > 0:
                ...
                delta = 0
                if fv > 0: delta = ((curr - fv) / fv * 100)
                
                color = "inverse" if fv > 0 and curr < fv else "normal"
                k5.metric("القيمة العادلة", f"{fv:.2f}", delta=f"{delta:.1f}%", delta_color=color)
            else:
                st.warning("تعذر جلب البيانات المالية حالياً (قد يكون الرمز غير مدعوم بالكامل أو المصدر لا يستجيب).")
