...
def view_dashboard(fin):
    ...
    # صندوق تاسي الأزرق
    st.markdown(f"<div class='tasi-card'><div><div style='opacity:0.9'>المؤشر العام (TASI)</div><div style='font-size:2.5rem;font-weight:900;'>{safe_fmt(tp)}</div></div><div style='background:rgba(255,255,255,0.2);padding:8px 20px;border-radius:10px;font-weight:bold;direction:ltr;'>{tc:+.2f}%</div></div>", unsafe_allow_html=True)
    
    # البطاقات الحيوية مع أيقونات
...
# === المحفظة (التصميم مطابق للصور 522 و 524) ===
def view_portfolio(fin, key):
    ...
    with t1:
        if not open_df.empty:
            ...
            render_custom_table(open_df, cols)
...
def view_cash_log():
    ...
    # 1. شريط الفرز (كما في الصورة)
    st.selectbox("فرز السجلات حسب:", ["التاريخ (الأحدث)", "المبلغ (الأعلى)"])
    
    # 2. التبويبات مع الأيقونات والإجمالي
    ...
    with t1:
        st.markdown(f"<h5 style='text-align:left; color:#059669;'>الإجمالي: {fin['deposits']['amount'].sum():,.2f}</h5>", unsafe_allow_html=True)
        render_custom_table(fin['deposits'], cols_simple)
        # زر الإضافة (بدون Expand More)
    ...
    with t2:
        st.markdown(f"<h5 style='text-align:left; color:#DC2626;'>الإجمالي: {fin['withdrawals']['amount'].sum():,.2f}</h5>", unsafe_allow_html=True)
    ...
    with t3:
        st.markdown(f"<h5 style='text-align:left; color:#2563EB;'>الإجمالي: {fin['returns']['amount'].sum():,.2f}</h5>", unsafe_allow_html=True)
        render_custom_table(fin['returns'], cols_simple + [('symbol', 'الرمز', 'text')])
        with st.expander("💵 إضافة عائد جديد"):
            with st.form("r"):
