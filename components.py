...
def render_table(df, cols_def):
    if df.empty: st.info("لا توجد بيانات للعرض"); return
    C = DEFAULT_COLORS
    ...
    for _, row in df.iterrows():
        ...
        for k, _ in cols_def:
            ...
            # === هنا الإصلاح الجذري للتقريب ===
            # أضفت net_sales و realized_gain للقائمة ليتم تقريبها إجبارياً
            elif k in ['gain', 'gain_pct', 'daily_change', 'return_pct', 'net_sales', 'realized_gain', 'amount']:
                try:
                    num_val = float(val)
                    c = C['success'] if num_val >= 0 else C['danger']
                    # إذا كان الرقم نسبة مئوية نضيف %
                    suffix = "%" if 'pct' in k or 'change' in k else ""
                    # إذا لم يكن نسبة، لونه عادي أو حسب الربح
                    style_color = f"color:{c};" if 'pct' in k or 'gain' in k or 'change' in k else ""
                    disp = f"<span style='{style_color} direction:ltr; font-weight:bold;'>{num_val:,.2f}{suffix}</span>"
                except: disp = val

            # تقريب المبالغ المالية العادية (مثل التكلفة والسعر)
            elif k in ['market_value', 'total_cost', 'entry_price', 'current_price', 'exit_price']:
                try: disp = f"{float(val):,.2f}"
                except: disp = val
            
            # الكميات (بدون فواصل عشرية)
            elif k == 'quantity':
