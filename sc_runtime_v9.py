...
- analysis history periods are derived from the resolved interval, avoiding the
  historic 1M minute/month ambiguity;
...
from persistent_market_cache_v8 import save_history, save_quote
from sc_runtime_v8 import install_sc_runtime_v8
from sc_runtime_v8 import runtime_status as _runtime_status_v8
...
def _drain_persistent_writes() -> None:
    ...
    while True:
        ...
        try:
            if kind == "history":
                save_history(key, value, ttl_seconds=ttl_seconds)
            else:
                save_quote(str(key), value, ttl_seconds=ttl_seconds)
        except Exception:
...
def _enqueue_persistent(
    ...
    queue_key = (kind, key)
    with _PERSIST_LOCK:
        # Replace an older pending snapshot for the same key instead of building
        # an unbounded database queue during a busy market refresh.
...
def _install_corrected_quote_batch() -> None:
    ...
    def fetch_batch_data(symbols_list: list) -> dict[str, dict[str, Any]]:
        ...
        # Fresh/stale values are returned instantly when every symbol is covered.
        # A mixed batch waits only because one or more requested symbols have no
        # usable value at all; the wait remains bounded by the quote budget.
        if not missing:
            return output
...
def runtime_status() -> dict[str, Any]:
    status = dict(_runtime_status_v8())
    with _PERSIST_LOCK:
...
def install_sc_runtime_v9() -> None:
    ...
    with _INSTALL_LOCK:
        if _INSTALLED:
            return
        # Standalone report consumers do not pass through app.py, so install the
        # same foundations explicitly before V8 captures module-level functions.
