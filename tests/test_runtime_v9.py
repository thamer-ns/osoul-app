...
def test_mixed_quote_batch_fetches_the_uncached_subset(monkeypatch):
    ...
    monkeypatch.setattr(runtime, "_store_quote", lambda symbol, payload: runtime._QUOTE_CACHE.__setitem__(runtime._normalized_symbol(symbol), (time.monotonic(), dict(payload))))
...
def test_persistent_history_write_never_blocks_request_thread(monkeypatch):
    ...
    monkeypatch.setattr(v9, "save_history", slow_save)
...
def test_role_reversal_with_required_weak_volume_stays_forming(monkeypatch):
    ...
    result = breakout.analyze_breakout_patterns(pd.DataFrame(), symbol="AAPL")
