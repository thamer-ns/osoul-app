...
def get_manager():
    """Return cookie manager instance or None.

    We keep it defensive so the app never crashes if the component is missing
    or fails on a specific deployment.
    """
...
def _cm_get(cm, name: str):
    if not cm:
        return None
    try:
        # بعض الإصدارات تدعم keyword arg
        try:
            return cm.get(cookie=name)  # type: ignore
        except TypeError:
            return cm.get(name)  # type: ignore
    except Exception:
        return None


def _cm_set(cm, name: str, value: str, expires_at):
    if not cm:
        return False
    try:
        # نضع key فريد لتجنب تعارض keys داخل Streamlit
        k = f"ck_{name}_{int(time.time()*1000)}"
        try:
            cm.set(cookie=name, val=value, expires_at=expires_at, key=k)  # type: ignore
        except TypeError:
            cm.set(name, value, expires_at, key=k)  # type: ignore
        return True
    except Exception:
        return False


def _cm_delete(cm, name: str):
    if not cm:
        return False
    try:
        # بعض الإصدارات تدعم delete مباشرة
        try:
            cm.delete(cookie=name)  # type: ignore
            return True
        except TypeError:
            try:
                cm.delete(name)  # type: ignore
                return True
            except Exception:
                pass
        except Exception:
        ...
        # fallback: set expired
        past = datetime.datetime.utcnow() - datetime.timedelta(days=1)
        return _cm_set(cm, name, "", past)
    except Exception:
        return False
...
def _get_cookie_user(cookie_manager):
    return _cm_get(cookie_manager, "osoul_user")
...
def _bootstrap_auth_from_cookie():
    ...
    cookie_token = None
    try:
        cookie_token = _norm(_cm_get(cm, "osoul_auth_v2"))
    except Exception:
        cookie_token = None

    if not cookie_token:
        try:
            cookie_token = _norm(_cm_get(cm, "osoul_auth"))
        except Exception:
            cookie_token = None
                ...
                if st.form_submit_button("دخول", type="primary"):
                    ...
                    if db_verify_user(u, p):
                        ...
                        try:
                            ...
                            cm = get_manager()
                            _cm_set(cm, "osoul_auth_v2", token, expires)
                            # توافق رجعي: الكوكي القديمة
                            _cm_set(cm, "osoul_auth", token, expires)
                            _cm_set(cm, "osoul_user", u, expires)
                        except Exception:
                            pass
...
def logout():
    try:
        _cm_delete(get_manager(), "osoul_user")
        _cm_delete(get_manager(), "osoul_auth_v2")
        _cm_delete(get_manager(), "osoul_auth")
    except Exception:
        pass
