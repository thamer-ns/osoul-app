...
# ==============================================================
# 🩺 Yahoo Diagnostics (UI-safe wrappers)
# ==============================================================

def get_last_yahoo_diagnostics():
    """UI wrapper: return last Yahoo diagnostics dict if available."""
    try:
        from financial_analysis.yahoo_data import get_last_yahoo_diagnostics as _get  # type: ignore
        d = _get()
        # Ensure plain dict (no numpy types)
        return dict(d) if isinstance(d, dict) else {"error": "invalid diagnostics type"}
    except Exception:
        return {"error": "diagnostics unavailable", "hint": "Diagnostics غير متاحة"}


def diagnose_yahoo_quote_summary(symbol: str):
    """UI wrapper: run a lightweight Yahoo quoteSummary diagnostics for a symbol."""
    try:
        from financial_analysis.yahoo_data import diagnose_quote_summary as _diag  # type: ignore
        d = _diag(symbol)
        return dict(d) if isinstance(d, dict) else {"error": "invalid diagnostics type"}
    except Exception:
        return {"error": "diagnostics unavailable", "hint": "Diagnostics غير متاحة"}
