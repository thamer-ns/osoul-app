"""Fast runtime router for official market providers with one legacy fallback."""
...
def _years_from_request(
    period: str | None,
    years: int | None,
    interval: str,
) -> int:
    """Honor the requested period before the compatibility ``years`` default."""
    ...
    if raw.endswith("d"):
        try:
            ...
            return max(1, min(5, (days + 364) // 365))
        except (TypeError, ValueError):
            return 1
    if raw:
        return 1
...
def _merge_attempts(
    ...
    *,
    ...
    lineage["provider_fusion_version"] = "5.1"
...
def install_market_data_router() -> None:
    ...
    def get_chart_history(
        ...
        if isinstance(fallback, pd.DataFrame) and not fallback.empty:
            attrs = dict(getattr(fallback, "attrs", {}) or {})
            lineage = dict(attrs.get("data_lineage") or {})
            source = str(
                lineage.get("source")
                or attrs.get("source")
    ...
    def fetch_batch_data(
        ...
        for raw_symbol in requested:
            ...
            if payload:
                ...
                store_aliases(output, raw_symbol, normalized, payload)
            else:
        ...
        for raw_symbol in failed:
            ...
            payload.setdefault(
                "prev_close",
                payload.get("previous_close") or 0.0,
            )
            payload.setdefault(
                "previous_close",
                payload.get("prev_close") or 0.0,
            )
            payload.setdefault(
                "change_pct",
            ...
            payload.setdefault(
                "change_available",
                payload.get("change_pct") is not None,
            )
            payload.setdefault("source", "failed")
            payload.setdefault("fetched_at", now)
            payload.setdefault(
                "is_stale",
                not bool(payload.get("price")),
            )
            payload["provider_attempts"] = (
                failed_attempts.get(raw_symbol, [])
                + list(payload.get("provider_attempts") or [])
            )
            payload["provider_fusion_version"] = "5.1"
            store_aliases(output, raw_symbol, normalized, payload)
        return output

    def get_tasi_history(
        period: str | None = None,
        interval: str = "1d",
    ) -> pd.DataFrame:
        return md.get_chart_history(
            "TASI",
            period=period,
            interval=interval,
            years=15,
    ...
    def get_analysis_sources(symbol: str) -> dict[str, Any]:
        ...
        if callable(original_sources):
            try:
                ...
                LOGGER.debug(
                    "Legacy source diagnostics failed",
                    exc_info=True,
                )
        normalized = md.get_ticker_symbol(symbol)
        quote_batch = md.fetch_batch_data([normalized])
        quote = (
            quote_batch.get(normalized)
            or quote_batch.get(str(normalized).upper())
            or {}
        )
        history = md.get_chart_history(
            normalized,
            period="2y",
            interval="1d",
        )
        attrs = (
            dict(getattr(history, "attrs", {}) or {})
            if isinstance(history, pd.DataFrame)
            else {}
        )
        lineage = dict(attrs.get("data_lineage") or {})
        base.setdefault("symbol", normalized)
        base["provider_fusion"] = {
            ...
            "history_attempts": list(
                lineage.get("provider_attempts") or []
            ),
        }
        return base

    get_chart_history._osoli_original = original_history
    fetch_batch_data._osoli_original = original_batch
    ...
    md._provider_fusion_version = "5.1"
    _INSTALLED = True


__all__ = ["_years_from_request", "install_market_data_router"]
