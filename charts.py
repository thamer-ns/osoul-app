...
def render_technical_chart(symbol, period, interval):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['BB_Upper'], line=dict(color='rgba(128,128,128,0.5)', width=1), name='BB Upper'), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df['BB_Lower'], line=dict(color='rgba(128,128,128,0.5)', width=1), fill='tonexty', name='BB Lower'), row=1, col=1)
    ...
    fig.add_bar(x=df.index, y=(df['MACD'] - df['Signal_Line']), name='Hist', marker_color='gray', row=4, col=1)
