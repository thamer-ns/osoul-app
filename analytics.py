...
def calculate_portfolio_metrics():
    ...
    # حماية من الجداول الفارغة
    if trades.empty:
        return {**sql, "market_val_open": 0, "unrealized_pl": 0, "cash": 0, "all_trades": pd.DataFrame(), "deposits": dep, "withdrawals": wit, "returns": ret}

    # تحويل الأرقام
    for c in ['quantity', 'entry_price', 'current_price', 'exit_price']:
        if c in trades.columns: trades[c] = pd.to_numeric(trades[c], errors='coerce').fillna(0.0)

    open_t = trades[trades['status'] == 'Open']
    mkt_val = (open_t['quantity'] * open_t['current_price']).sum()
    unrealized = mkt_val - sql['cost_open']
    
    # حساب الكاش الدقيق
    sales = trades[trades['status'] == 'Close'].apply(lambda x: x['quantity'] * x['exit_price'], axis=1).sum()
    ...
    return {**sql, "market_val_open": mkt_val, "unrealized_pl": unrealized, "cash": cash, "all_trades": trades, "deposits": dep, "withdrawals": wit, "returns": ret}
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            for s, data in d.items():
                if data['price'] > 0:
                    cur.execute("UPDATE Trades SET current_price=%s, prev_close=%s WHERE symbol=%s AND status='Open'", (data['price'], data['prev_close'], s))
            conn.commit()
