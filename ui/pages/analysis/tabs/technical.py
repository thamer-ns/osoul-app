...
def render(symbol: str):
    try:
        ...
        if mode == "احترافي":
            if hasattr(v, "_render_tv_like_chart"):
                v._render_tv_like_chart(symbol, period_opts[p_label], interval_opts[i_label])
            else:
                st.warning("⚠️ _render_tv_like_chart غير متوفر، سيتم استخدام الشارت القديم.")
                if hasattr(v, "_render_technical_chart_flex"):
                    v._render_technical_chart_flex(symbol, period=period_opts[p_label], interval=interval_opts[i_label])
        else:
            if hasattr(v, "_render_technical_chart_flex"):
                v._render_technical_chart_flex(symbol, period=period_opts[p_label], interval=interval_opts[i_label])
            else:
                st.error("❌ الشارت القديم غير متوفر داخل views_impl.")
    except Exception as e:
        st.error(f"❌ تعذر فتح تبويب الفني: {e}")
