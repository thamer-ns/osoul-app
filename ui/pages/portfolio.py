...
from ui.common import safe_status_series, clean_symbols_list, normalize_symbol
...
def view_portfolio(fin, key):
    ...
    status = safe_status_series(sub) if not sub.empty else pd.Series([], dtype=str)
    if len(status):
    ...
    with t1:
        ...
        if not op.empty:
            ...
            symbols = clean_symbols_list(op["symbol"].astype(str).tolist()) if "symbol" in op.columns else []
            try:
            ...
            if "symbol" in op.columns:
                op["symbol"] = op["symbol"].astype(str).apply(normalize_symbol)
