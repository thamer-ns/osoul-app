...
def router():
    ...
    if pg == "home":
        ...
        view_tools()
    elif pg == "settings":# views_impl.py
    ...
    try:
        ...
        if raw and isinstance(raw[0], (tuple, list)):
            ...
            for item in raw:
                ...
                strat_map[label] = k
            if strat_map:
                label = st.selectbox("اختر الاستراتيجية", list(strat_map.keys()), index=0, key="lab_strat_label")
                strat = strat_map[label]
            ...
            raw_str = [str(x) for x in raw] if raw else ["Trend", "Sniper"]
            strat = st.selectbox("اختر الاستراتيجية", raw_str, index=0, key="lab_strat")
    except Exception:
        strat = "Trend"
